...
public class BST<E extends Comparable<E>> implements BSTInterface<E> {
    ...
    public E findLCA(Set<E> elements) {
        ...
        E minCommon = null;
        for (int i = 0; i < paths.size(); i++) {//percorrer a lista de listas
            for (int j = 0; j < paths.get(i).size(); j++) {//percorrer a lista i
                if (containsElement(paths, paths.get(i).get(j))) {//verficar se o elemento j está presente em todos os paths
                    minCommon = paths.get(i).get(j);//se estiver presente é guardado
                }
    ...
    private boolean containsElement(List<List<E>> paths, E element) {
        for (List<E> p : paths) {
    ...
    public boolean isSymmetric() {
        Map<Integer, List<E>> map = nodesByLevel();

        for (Map.Entry<Integer, List<E>> p : map.entrySet()) {

            if (p.getValue().size() > 1) {
                List<E> l1 = new LinkedList<>();
                List<E> l2 = new LinkedList<>();
                for (E a : p.getValue()) {
                    if (l1.size() != p.getValue().size() / 2) {
                        l1.add(a);
                    } else {
                        l2.add(a);
                    }
                ...
                for (int i = 0; i < l1.size(); i++) {
                    if (!l1.get(i).equals(Integer.parseInt("-" + l2.get(l2.size() - 1 - i)))) {
                        return false;
                    }
                }
            }
    ...
    //Ex3-Exame 2022-Época Normal
    public List<E> travessiaEspiral() {
        Map<Integer, List<E>> map = this.nodesByLevel();
        List<E> list = new LinkedList<>();
        int limit1 = 0;
        int limit2 = map.size() - 1;
        while (true) {
            if (map.size() % 2 == 0) {
                //se o tamanho da tree for par
                List<E> decrescente = map.get(limit1);
                decrescente.sort(Comparator.reverseOrder());
                list.addAll(decrescente);
                list.addAll(map.get(limit2));
                limit1++;
                limit2--;
                if (limit1 + 1 != limit2) {
                    return list;
                }
            } else {//se o tamanho da tree for ímpar
                if (limit1 == limit2) {
                    List<E> decrescent = map.get(limit1);
                    decrescent.sort(Comparator.reverseOrder());
                    list.addAll(decrescent);
                    return list;
                }
                List<E> decrescente = map.get(limit1);
                decrescente.sort(Comparator.reverseOrder());
    ...
    //Ex3 2021-2022_Recurso
    public List<List<E>> allPaths() {
        List<E> allElements = (List<E>) inOrder();
        List<E> terminalNodes = new LinkedList<>();
