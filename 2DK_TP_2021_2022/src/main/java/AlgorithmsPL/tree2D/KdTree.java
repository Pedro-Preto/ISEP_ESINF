...
public class KdTree<T> {
    ...
    private final Comparator<Node<T>> cmpX = new Comparator2DTreeX<>();


    private final Comparator<Node<T>> cmpY = new Comparator2DTreeY<>();
    ...
    public T findNearestNeighbour(double x, double y) {
        return findNearestNeighbour(root, x, y);
    }

    private T findNearestNeighbour(Node<T> fromNode, final double x, final double y) {
        return new Object() {
        ...
        }.findNearestNeighbour(fromNode, true);
    }
    ...
    public List<T> findNearestNeighbours(final double x, final double y, final int limit) {
        ...
        final Comparator<Node<T>> cmp = (node1, node2) -> {
