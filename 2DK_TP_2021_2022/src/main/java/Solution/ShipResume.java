...
import Utils.DiferenceBetweenToDates;
...
public class ShipResume {
    ...
    public void getShipResume(int mMSI, BST<ShipMMSI> tree) {

        List<ShipMMSI> list = tree.find(new ShipMMSI(mMSI)).getMapList();

        System.out.printf("MMSI:%s%nIMO:%s%nCall Sign:%s%nVessel Name:%s%nStart Base Date Time:%s%nEnd Base Date Time:%s%nTotal Movements Time:%s%nTotal Number Of Movements:%s%nMax SOG:%s%nMean SOG:%s%nMax COG:%s%nMean COG:%s%nDeparture Latitude:%s%nDeparture Longitude:%s%nArrival Latitude:%s%nArrival Longitude:%s%nTravelled Distance:%s%nDelta Distance:%s%n %n",
